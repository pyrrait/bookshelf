...
function Rating({listItem, user}) {
  ...
  // 💰 the mutate function should make a PUT request to the list-items/:listItemId endpoint
  // with the data and the user token
  // 💰 if you want to make sure the list items are updated, then use the
  // `onSettled` config option to call queryCache.refetchQueries('list-items')
  const update = () => {}
  const error = null
